...
from apps.app import cache, mdb_user
...
def get_one_user(user_id=None, username=None, email=None, mphone_num=None):
    ...
    if user_id:
        ...
        return {}
    user = mdb_user.db.user.find_one(q)
    if user:
...
def update_one_user(user_id, updata):
    ...
    r = mdb_user.db.user.update_one({"_id":ObjectId(user_id)}, updata)
...
def delete_one_user(user_id):
    ...
    :return:
    '''
    user = mdb_user.db.user.find_one({"user_id": ObjectId(user_id)})
    r = mdb_user.db.user.delete_one({"_id": ObjectId(user_id)})
    if r.deleted_count:
        clean_get_one_user_cache(user=user)
        return r
    else:
...
def clean_get_one_user_cache(user_id=None, user=None):
    ...
    if user:
        cache.delete_autokey(fun=fun_name, db_type="redis", user_id=str(user["_id"]))
        cache.delete_autokey(fun=fun_name, db_type="redis", username=user["username"])
