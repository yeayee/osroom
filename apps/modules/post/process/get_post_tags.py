...
def get_tags():
    ...
    # 查看是否存在缓存
    cache_key = cache.get_autokey( fun="_get_tags", key_base64=False, db_type="redis",
                                   user_id=user_id,last_days=last_days,
                                   tlimit=tlimit, sort=sort)
...
def _get_tags(user_id, last_days, tlimit, sort):
    ...
    query_conditions = {
        ...
        "issued":1,
        "is_delete":0,
        "audit_score":{"$lt": get_config("content_inspection", "ALLEGED_ILLEGAL_SCORE")}
    }
    if user_id:
    ...
    # 保留一份长期缓存
    cache.set(key="LAST_POST_TAGS_CACHE", value=data, ex=3600 * 24 * 7, db_type="mongodb")
    return data