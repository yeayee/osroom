...
def add_display_setting():
    ...
    if category_id and category_id.lower() != "default":
        display_setting_category = mdb_web.db.category.find_one(
    ...
    if not batch and mdb_sys.db.theme_display_setting.find_one(
...
def edit_display_setting():
    ...
    old_display_setting = mdb_sys.db.theme_display_setting.find_one(
        ...
        if data["msg_type"] != "s":
            ...
            if "keys" in data:
                for key in data["keys"]:
                    temp_url = key
                if temp_url:
                    mdb_sys.db.theme_display_setting.update_one({"_id": ObjectId(
                        display_setting_id), "user_id": user_id}, {"$set": {"url": temp_url}})
                    data = {
    ...
    if mdb_sys.db.theme_display_setting.find_one(query):
        ...
        r = mdb_sys.db.theme_display_setting.update_one(
            {"_id": ObjectId(display_setting_id), "user_id": user_id}, {"$set": info})
        if r.modified_count:
            data = {
